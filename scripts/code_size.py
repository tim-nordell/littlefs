...
# ./scripts/code_size.py -qo old.csv
# while true ; do ./scripts/code_size.py -d old.csv -o new.csv ; git diff --exit-code --quiet && cp new.csv old.csv ; inotifywait -rqe modify * ; done
#
# Or both at the same time:
# ./scripts/code_size.py -qo old.csv && cp old.csv commit.csv
# while true ; do ( ./scripts/code_size.py -i commit.csv -d old.csv -s ; ./scripts/code_size.py -d old.csv -o new.csv -s ) 2>/dev/null | awk 'BEGIN {printf "%-16s %7s %7s %7s\n","","old","new","diff"} (NR==2 && $1="commit") || (NR==4 && $1="new") {printf "%-16s %7s %7s %7s %s\n",$1,$2,$3,$4,$5,$6}' ; git diff --exit-code --quiet && cp new.csv old.csv ; inotifywait -rqe modify * ; done
#
